			...
			<form action="" id="invoice-form">
				...
				<div class="row">
					<div class="col-md-6">
						...
						<div class="form-group">
							<label for="customer_name" class="control-label">Nombre Cliente</label>
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<option value="2" <?php echo isset($type) && $type == 2 ? 'selected' : '' ?>>Servicios</option>
							</select>
						</div>
					</div>
				</div>
				...
				<div class="row align-items-end">
					...
					<div class="col-md-3">
						<div class="form-group">
						<label for="form_id" class="control-label">Producto/Servicio</label>
							<select  id="form_id" class="custom-select custom-select-sm select2">
								<option selected="" disabled>Seleccione la categoría primero</option>
								<?php 
								$data_json = array();
									if($type == 1):
					...
					<div class="col-md-2 pb-1">
						<div class="form-group">
							<button class="btn btn-flat btn-primary" type="button" id="add_item"><i class="fa fa-plus"></i> Agregar</button>
						</div>
					</div>
				</div>
				<div class="row">
...
var item_arr = $.parseJSON('<?php echo json_encode($item_arr) ?>');
	function generate_code(){
		...
		$.ajax({
			...
			error:err=>{
				console.log(err)
				alert_toast(' Ocurrió un error al generar el código de factura','error')
			},
	...
	function add_item($obj = null){
		...
		if($('#item-list tbody').find('[name="form_id[]"][value="'+form_id+'"]').length > 0){
			alert_toast(' El artículo ya está en la lista.','warning')
			end_loader();
			return false;
		}
		...
		// details column
			td = $("<td>")
			td.html("<p class='m-0'><small><b>Categoría:</b> "+category+"</small></p>"+
					"<p class='m-0'><small><b>Nombre: </b>"+form_name+"</p>"+
	...
	$(document).ready(function(){
		...
		$('#add_item').click(function(){
			if($('#qty').val() == '' ||$('#unit').val() == '' || $('#form_id').val() == ''){
				alert_toast("Primero complete el formulario del artículo.",'warning')
				return false;
			}
			add_item()
		})
			...
			$('[name="invoice_code"]').on('input',function(){
				...
				success:function(resp){
					if(resp.status == 'available'){
						...
						console.log(resp)
						alert_toast(' Se produjo un error al validar el código de la factura.','error')
					}
					end_loader(); 
				}
			})
		...
		$('#invoice-form').submit(function(e){
			...
			if($('#item-list tbody tr').length <= 0){
				alert_toast("No se enumeran artículos.",'warning')
				return false;
			}
			if($('[name="invoice_code"]').hasClass('border-danger') == true){
				alert_toast("El código de factura ya existe.",'warning')
				return false;
			}
